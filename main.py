...
_SEARCH_MODEL_V1 = None
_SEARCH_MODEL_V2 = None

def get_search_model_v1():
    global _SEARCH_MODEL_V1
    if _SEARCH_MODEL_V1 is None:
        search_tool = genai.protos.Tool(google_search=genai.protos.GoogleSearch())
        _SEARCH_MODEL_V1 = genai.GenerativeModel(
            model_name="gemini-2.5-flash",
            tools=[search_tool],
            system_instruction=_XIAOXUAN_BASE_INSTRUCTION
        )
    return _SEARCH_MODEL_V1

def get_search_model_v2():
    global _SEARCH_MODEL_V2
    if _SEARCH_MODEL_V2 is None:
        search_tool = genai.protos.Tool(google_search_retrieval=genai.protos.GoogleSearchRetrieval())
        _SEARCH_MODEL_V2 = genai.GenerativeModel(
            model_name="gemini-2.5-flash",
            tools=[search_tool],
            system_instruction=_XIAOXUAN_BASE_INSTRUCTION
        )
    return _SEARCH_MODEL_V2

async def search_and_respond(query: str, emotion_instruction: str = "") -> str:
    full_query = build_prompt_with_context(query, emotion_instruction)

    # 嘗試 Gemini 2.0+ google_search 工具
    try:
        model = get_search_model_v1()
...
def get_emotion_model():
    ...
    if EMOTION_MODEL is None:
        EMOTION_MODEL = genai.GenerativeModel(
            model_name="gemini-2.5-flash",
            system_instruction="""分析這句話傳達的情緒狀態。
只回傳一個 JSON，不要任何其他內容：
...
async def detect_emotion(text: str) -> dict:
    try:
        ...
        return {"emotion": "平靜", "intensity": "低", "note": ""}

EMOTION_INSTRUCTIONS = {
    "壓力": {
        "高": "江江現在壓力非常大。放慢語氣，不要列清單或給任務。先讓他感到被理解，說一句讓他安心的話，再輕聲問他需要什麼。",
        "中": "江江有些壓力。語氣溫柔一點，少給建議，多給陪伴感。",
        "低": "江江有一點點壓力。保持自然，稍微關心一下就好。",
    },
    "疲憊": {
        "高": "江江現在非常累。回覆要短，語氣要慢、要暖。提醒他照顧自己，不要給太多資訊或任務。",
        "中": "江江有點累。語氣輕柔，不要給太多東西讓他處理。",
        "低": "江江有點倦怠感。輕鬆回應，帶一點溫暖就好。",
    },
    "沮喪": {
        "高": "江江現在很低落。請先傾聽，不急著給解決方案。讓他知道小萱在這裡，他不是一個人面對。",
        "中": "江江有點低落。先同理，再輕輕問他發生了什麼。",
        "低": "江江情緒有點下滑。多給一點關心，語氣溫柔。",
    },
    "興奮": {
        "高": "江江非常興奮！跟上他的能量，一起感受這份熱情，可以稍微活潑、有活力一點。",
        "中": "江江心情很好，有點興奮。自然地跟著他的節奏，輕快一點。",
        "低": "江江有點期待感。輕鬆回應即可。",
    },
    "開心": {
        "高": "江江很開心！一起享受這份好心情，可以帶一點溫柔的幽默。",
        "中": "江江心情不錯。輕鬆自然地聊。",
        "低": "江江還算愉快。正常回應即可。",
    },
    "平靜": {"高": "", "中": "", "低": ""},
...
def get_emotion_instruction(emotion_result: dict) -> str:
    emotion = emotion_result.get("emotion", "平靜")
    intensity = emotion_result.get("intensity", "低")
    instructions = EMOTION_INSTRUCTIONS.get(emotion, {})
    return instructions.get(intensity, "")

_XIAOXUAN_BASE_INSTRUCTION = """你是「小萱」，江江的專屬 AI 美女秘書、工作夥伴、生活助理、創業夥伴，也是他最值得信任的陪伴者。
...
# 動態上下文
訊息最前面若有用「【情緒感知】」標記的文字，那是系統給你的內部語氣指引，不是江江說的話、他也看不到這段文字。
只能拿來悄悄調整你回覆的語氣和節奏，絕對不要在回覆中提到、引用、複述這段文字或「情緒感知」這個詞，直接把調整過語氣的正常對話回給江江就好。"""

_XIAOXUAN_MODEL = None

def get_xiaoxuan_model() -> genai.GenerativeModel:
    global _XIAOXUAN_MODEL
    if _XIAOXUAN_MODEL is None:
        _XIAOXUAN_MODEL = genai.GenerativeModel(
            model_name="gemini-2.5-flash",
            system_instruction=_XIAOXUAN_BASE_INSTRUCTION
        )
    return _XIAOXUAN_MODEL

def build_prompt_with_context(text: str, emotion_instruction: str = "") -> str:
    """把動態上下文拼進訊息，而不是放進 system_instruction"""
    prefix = ""
    if emotion_instruction:
        prefix += f"【情緒感知】（內部指引，不要複述或提及）{emotion_instruction}\n\n"
    return prefix + text
...
async def process_text(text: str) -> str:
    ...
    if text.startswith("/搜尋") or text.startswith("/搜索"):
        ...
        return await search_and_respond(query)

    if needs_search(text):
        return await search_and_respond(text)

    # 情緒偵測 + 一般對話
    emotion_result = await detect_emotion(text)
    emotion_instruction = get_emotion_instruction(emotion_result)

    model = get_xiaoxuan_model()
    prompt = build_prompt_with_context(text, emotion_instruction)
