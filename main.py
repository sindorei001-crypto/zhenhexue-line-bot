...
def build_default_model(memory_context: str = "", emotion_instruction: str = "") -> genai.GenerativeModel:
...
async def process_text(text: str) -> str:
    ...
    # 高強度負面情緒自動存入記憶
    if emotion_result.get("intensity") == "高" and emotion_result.get("emotion") in ["壓力", "疲憊", "沮喪"]:
        note = emotion_result.get("note", "")
        if note:
            asyncio.create_task(asyncio.to_thread(
                save_memory, "情境", "情緒狀態", f"近期狀態：{note}"
            ))

    model = build_default_model(memory_context, emotion_instruction)
